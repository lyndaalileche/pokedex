  ...
  import Loading from '../loading'; // Importer le composant LoadingModule '"next/image"' has no exported member 'Image'. Did you mean to use 'import Image from "next/image"' instead?ts(2614)
  ...
  const PokemonDetailsPage: React.FC = () => {
    ...
    return (
      <div style={{ textAlign: 'center', fontSize: '20px', color: '#333', backgroundColor: '#f4f4f4', borderRadius: '10px', padding: '20px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)' }}>
        <h1 style={{ color: '#3f51b5', marginBottom: '20px', textTransform: 'capitalize' }}>{pokemonDetails.name}</h1>
        <p>ID : {pokemonDetails.id}</p>
        <p>Types : {pokemonDetails.types.map((type, index) => <span key={index} style={{ backgroundColor: '#e0e0e0', borderRadius: '5px', padding: '2px 6px', marginRight: '5px' }}>{type.type.name}</span>)}</p>
        <Image src={pokemonDetails.sprites.front_default} alt={`${pokemonDetails.name} sprite`} width={200} height={200} style={{ borderRadius: '50%', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)', marginBottom: '20px' }} />
        <p>Description: {pokemonDetails.description}</p>
        <p style={{ color: '#fff', backgroundColor: '#3f51b5', borderRadius: '5px', padding: '5px', margin: '10px 0' }}>Poids : {pokemonDetails.weight / 10} kg</p>
        <p style={{ color: '#fff', backgroundColor: '#3f51b5', borderRadius: '5px', padding: '5px', margin: '10px 0' }}>Taille : {pokemonDetails.height / 10} m</p>
        <p>Capacités : {pokemonDetails.abilities.map((ability, index) => <span key={index} style={{ backgroundColor: '#f44336', color: '#fff', borderRadius: '5px', padding: '2px 6px', marginRight: '5px' }}>{ability}</span>)}</p>
